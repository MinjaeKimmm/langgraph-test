...
def critique(state: AgentState, config):
    ...
    model = _get_model(config, "openai", "critique_model").bind_tools([Accept])
    response = model.invoke(messages)
    print(response)  # Debugging: Check response structure

    # Extract tool call arguments manually
    if hasattr(response, "tool_calls") and response.tool_calls:
        tool_call = response.tool_calls[0]  # Get first tool call
        tool_args = tool_call["args"]  # Extract function arguments
        logic = tool_args.get("logic", "")
        accepted = tool_call["name"] == "Accept"  # Assume tool name confirms acceptance
    else:
        logic = "No structured response received."
        accepted = False
